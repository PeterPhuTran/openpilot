...
class Events:
  def __init__(self, frogpilot=False):
    ...
    # FrogPilot variables
    self.event_map = FROGPILOT_EVENTS if frogpilot else EVENTS
    self.event_msg = custom.FrogPilotOnroadEvent if frogpilot else log.OnroadEvent
    self.event_names = FROGPILOT_EVENT_NAME if frogpilot else EVENT_NAME

    self.event_counters = dict.fromkeys(self.event_map.keys(), 0)
  ...
  def contains(self, event_type: str) -> bool:
    return any(event_type in self.event_map.get(e, {}) for e in self.events)
  ...
  def create_alerts(self, event_types: list[str], callback_args=None):
    ...
    for e in self.events:
      types = self.event_map[e].keys()
      for et in event_types:
        if et in types:
          alert = self.event_map[e][et]
          if not isinstance(alert, Alert):
            alert = alert(*callback_args)

          if DT_CTRL * (self.event_counters[e] + 1) >= alert.creation_delay:
            alert.alert_type = f"{self.event_names[e]}/{et}"
            alert.event_type = et
            ret.append(alert)
    return ret
  ...
  def to_msg(self):
    ...
    for event_name in self.events:
      event = self.event_msg.new_message()
      event.name = event_name
      for event_type in self.event_map.get(event_name, {}):
...
AlertCallbackType = Callable[[car.CarParams, car.CarState, messaging.SubMaster, bool, int, log.ControlsState, SimpleNamespace], Alert]
