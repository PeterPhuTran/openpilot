...
from openpilot.selfdrive.controls.lib.longitudinal_mpc_lib.long_mpc import LEAD_DANGER_FACTOR, desired_follow_distance, get_jerk_factor
...
STANDARD_ACCELERATION_JERK, _, STANDARD_SPEED_JERK = get_jerk_factor()
...
class FrogPilotTraffic:
  ...
  def update(self, sm):
    ...
    self.update_jerk(braking_severity, sm)
  ...
  def update_jerk(self, braking_severity, sm):
    self.acceleration_jerk = STANDARD_ACCELERATION_JERK
    self.speed_jerk = STANDARD_SPEED_JERK
